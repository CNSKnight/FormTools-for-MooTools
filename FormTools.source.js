...
move = function(fbox, tbox) {
     ...
     for(i=0; i<tbox.options.length; i++) {
          arrLookup[tbox.options[i].text] = tbox.options[i].get('value');
          arrTbox[i] = tbox.options[i].text;
     }
     var fLength = 0;
     var tLength = arrTbox.length;
     for(i=0; i<fbox.options.length; i++) {
          arrLookup[fbox.options[i].text] = fbox.options[i].get('value');
          if(fbox.options[i].selected && fbox.options[i].get('value') !== "") {
  ...
  //   arrTbox.sort();
     ...
     for (c=0; c<arrFbox.length; c++) {
          no = new Option();
          no.set('value', arrLookup[arrFbox[c]]);
          no.text = arrFbox[c];
          fbox[c] = no;
     }
     for (c=0; c<arrTbox.length; c++) {
     	no = new Option();
     	no.set('value', arrLookup[arrTbox[c]]);
     	no.text = arrTbox[c];
     	tbox[c] = no;
     }
};
...
submitForm = function(s) { s.set('value', 'ok, hang on...'); return true; };
submitFormEng = function(s)  { s.set('value', 'ok, hang on...'); return true; };
submitFormEsp = function(s)  { s.set('value', 'ok, mantente...'); return true; };
nameSelf = function() {
	self.name = selfname;
};

shift_form_vals = function(formid, keyfrom, keyto) {
	var shftform = typeof(document.forms[formid]) !== 'undefined' ? document.forms[formid] : $(formid);
	if (shftform) {
	    var idx, ff;
	    if (shftform.thecheck.checked === 1) {
	        for (idx in this.fieldChks) {
	            if (idx.indexOf(keyto) !== 0) { /*skip the 'to' elements*/
	                ff = idx.replace(keyfrom, keyto);
	                shftform[ff].set('value', shftform[idx].get('value'));
	            }
	        }
	    } else	if (shftform.thecheck.checked === 0) {
	        for (idx in this.fieldChks) {
	            if (! idx.indexOf(keyto) === 0) { /*skip the s_field_name elements*/
	                ff = keyfrom+idx;
	                shftform[ff].set('value', '');
	            }
	        }
	    }
	    return true;
